...
type rule struct {
	...
	fromEntities []v3.IdentitySelector
	toEntities   []v3.IdentitySelector
}
...
func (r *rule) sanitize() error {
	if r == nil || r.IdentitySelector.LabelSelector == nil {
	...
	r.fromEntities = []v3.IdentitySelector{}
	r.toEntities = []v3.IdentitySelector{}
	entities := []v3.Entity{}

	ingressEntityCounter := 0
	for _, rule := range r.Ingress {
		if rule.FromEntities != nil {
			entities = append(entities, rule.FromEntities.Entities...)
			ingressEntityCounter += len(rule.FromEntities.Entities)
		}
	}

	for _, rule := range r.Egress {
		if rule.ToEntities != nil {
			entities = append(entities, rule.ToEntities.Entities...)
		}
	}

	for j, entity := range entities {
		selector, ok := v3.EntitySelectorMapping[entity]
		if !ok {
	...
	for _, r := range r.Ingress {
		...
		if r.FromEntities != nil {
			for _, entity := range r.FromEntities.Entities {
				sel, ok := v3.EntitySelectorMapping[entity]
				if !ok {
					log.Warningf("Ignoring entity %s while checking if rules can reach since it is unknown", entity)
					continue
				}
				ctx.PolicyTrace("    Allows from labels %+v", sel)
				if sel.Matches(ctx.From) {
...
func (r *rule) canReachEntities(ctx *SearchContext, state *traceState) v3.Decision {
