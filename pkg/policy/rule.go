...
type rule struct {
	...
	fromEntities []v2.EndpointSelector
	toEntities   []v2.EndpointSelector
}
...
func (r *rule) sanitize() error {
	...
	r.fromEntities = []v2.EndpointSelector{}
	r.toEntities = []v2.EndpointSelector{}
	entities := []v2.Entity{}
	...
	for j, entity := range entities {
		selector, ok := v2.EntitySelectorMapping[entity]
		if !ok {
...
func (r *rule) canReachEntities(ctx *SearchContext, state *traceState) v2.Decision {
	for _, entitySelector := range r.toEntities {
		if entitySelector.Matches(ctx.To) {
			ctx.PolicyTrace("+     Found all required labels to match entity %s\n", entitySelector.String())
			state.matchedRules++
